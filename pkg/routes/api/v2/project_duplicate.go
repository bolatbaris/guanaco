...
func RegisterProjectDuplicateRoutes(api huma.API) {
	...
	Register(api, huma.Operation{
		...
		Description: "Deep-copies a project — its tasks, files, kanban data, comments, attachments, labels, relations and backgrounds — into a new project owned by the authenticated user. The user needs read access to the source project, plus write access to the parent project when one is given. The copy is placed under parent_project_id (top level if omitted). Returns the duplicate in duplicated_project.",
