...
type TaskListQueryParams struct {
	...
	SortBy             []string `query:"sort_by,explode" doc:"Fields to sort by (e.g. done, due_date). Repeatable; pair positionally with order_by. The special value relevance sorts by search relevance (most relevant first, requires s; ignored when the database cannot score the query)."`
